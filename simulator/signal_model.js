/**
 * 与 index.js 联调的简化物理层信号模型（路径损耗 + 阴影/快衰落）。
 * 单一入口，避免与 physical_layer.js（高阶传播模型）混用。
 */
...
function calculateFSPL(distanceMeters, frequencyHz) {
 ...
 * @param {number} totalDevices
 * @param {{ frequency?: number }} deviceConfig
 * @param {object} globalConfig
 * @param {number} timestamp
 * @param {{ x?: number, y?: number, z?: number }|null|undefined} deviceSpecificPosition
 */
function calculateRealisticSignal(
  deviceIdx,
  totalDevices,
  deviceConfig,
  globalConfig,
  timestamp,
  deviceSpecificPosition
) {
  ...
  const distance = calculateDistance(nodePos, gwPos) * PATH_LOSS_DISTANCE_MULTIPLIER;
  ...
  return {
    ...
    rssiStd: Math.round(
      Math.sqrt(Math.pow(model.shadowFadingStd, 2) + (model.fastFadingEnabled ? 4 : 0)) * 10
    ) / 10,
