      ...
      if (config.multiGateway && config.multiGateway.enabled) {
        ...
        if (selected.length > 0) {
          ...
          selected.forEach(gw => {
            ...
            const rxpkWithSignal = { ...rxpk, rssi: gw.rssi, loRaSNR: gw.snr };
            let gwPayload;
            if (mqttMarshaler === 'protobuf' && gwProto && gwProto.UplinkFrame) {
              try {
                const frame = gwProto.UplinkFrame.create({ phyPayload: Buffer.from(rxpkWithSignal.data, 'base64') });
                gwPayload = gwProto.UplinkFrame.encode(frame).finish();
              } catch (e) {
                gwPayload = JSON.stringify({ rxpk: [rxpkWithSignal] });
              }
            } else {
              gwPayload = JSON.stringify({ rxpk: [rxpkWithSignal] });
            }
